# Client Part 1 - Warmup and Main Phase

This client can run both the warmup phase (32K messages) and main phase (500K messages) to test the server.
...
1. **Start your server on EC2** (see Server README)

2. **Get your EC2 public IP/DNS**
   - Go to AWS Console → EC2 → Your instance
   - Copy the "Public IPv4 DNS" (like: ec2-52-12-34-56.us-west-2.compute.amazonaws.com)

3. **Update the URL in code:**
   - Open `WarmUpPhase.java` in your IDE
   - Find the line with `serverUrl = ...`
   - Replace with your EC2 address
   - Save the file

4. **Run it:**
   - Right-click on `WarmUpPhase.java`
   - Select "Run 'WarmUpPhase.main()'"
   - Watch the console output

5. **Repeat for Main Phase:**
   - Open `MainPhase.java`
   - Update the URL same way
   - Right-click and run
...
The output shows:
- How many messages sent successfully
- How many failed
...
### Main Phase:
1. Can run warmup first (or separately)
2. Creates a message generator thread
3. Generator creates all messages and puts them in a queue
4. Creates 100 sender threads
5. Each sender thread takes messages from queue and sends them
6. Keeps connections open (more efficient)
