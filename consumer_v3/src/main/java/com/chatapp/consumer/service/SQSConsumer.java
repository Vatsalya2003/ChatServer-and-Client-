...
public class SQSConsumer {
    ...
    public void initialize() {
        System.out.println("\n-------------------------------");
        System.out.println("Initializing OPTIMIZED SQS Consumer...");
        ...
        while (running) {
            try {
                ...
                if (!messages.isEmpty()) {
                    ...
                    // OPTIMIZATION: Collect receipt handles for batch delete
                    ...
                    for (Message message : messages) {
                        try {
                            ...
                            // ASYNC broadcast - doesn't block!
                            roomManager.broadcastToRoomAsync(roomId, queueMsg);

                            // Database add - instant (just queues it)
                            if (dbWriter != null) {
                                dbWriter.addMessage(queueMsg);
                            }
                            ...
                            deleteEntries.add(DeleteMessageBatchRequestEntry.builder()
                                    .id(String.valueOf(deleteEntries.size()))
                                    .receiptHandle(message.receiptHandle())
                                    .build());
                    ...
                    // OPTIMIZATION: Batch delete all at once
